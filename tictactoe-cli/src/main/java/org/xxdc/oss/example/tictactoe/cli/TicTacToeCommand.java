...
@CommandLine.Command(name = "tictactoe", description = "A Game of Tic-Tac-Toe", mixinStandardHelpOptions = true, version = "1.2.1", sortOptions = false)
public class TicTacToeCommand implements Runnable {
    ...
    static class PlayerOption {
        ...
        @CommandLine.Option(names = {"-p", "--player", "-a", "--algorithm"}, description = {
            "The player/algorithm type, one of @|bold ${COMPLETION-CANDIDATES}|@ (default: ${DEFAULT-VALUE}).",
            "- Example 1: `-p HUMAN -m X -p RANDOM -m O` will create a game with a human player and a random bot player.",
            "- Example 2: `-p HUMAN -m X -p MINIMAX -m X -p ALPHABETA -m W` will create a game with a human player and two bot players using minimax and alphabeta respectively.",
            "- Example 3: `-p MCTS -mi 1000 -m O -p RANDOM -m X` will create a game with a mcts bot player with a maximum number of iterations of 1000 vs. a random bot player.",
            "- Example 4: `-mO -mX` will create a game with two random bot players.",
            "- Example 5: `-mX -mO -p MCTS -mi 500` will create a game with a random bot and a mcts bot player with a maximum number of iterations of 500.",
            }, fallbackValue = "RANDOM", required = true, order = 2)
